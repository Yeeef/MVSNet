...
private:
	...
	*/
	...
	static void estimate(Estimator & paramEstimator, std::vector<T> &data,
											 int numForEstimate,
											 short *bestVotes, short *curVotes,
																 ...
																 bool test)
{
	...
	for (i = 0; i < numTries; i++) {
		...
		maxIndex = numDataObjects - 1;
		if (test)
			std::cout << i << " : " << numTries << " : " << "numForEstimate: " << numForEstimate << std::endl;
...
/*****************************************************************************/
template<class Estimator, class T, class S>
void Ransac::estimate(Estimator & paramEstimator, std::vector<T> &data,
											int numForEstimate, short *bestVotes, short *curVotes,
											int &numVotesForBest, int *arr)
{
	std::vector<T *> exactEstimateData;
